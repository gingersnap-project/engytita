...
public class CertUtil {
    ...
    * @throws ProxyException if there is any exception while reading the pem file
    */
   @SuppressWarnings({"UnstableApiUsage", "unchecked"})
   public static <T> T readPemFromFile(String pemFile) {
      try (PEMParser pemParser = new PEMParser(Files.newBufferedReader(Paths.get(pemFile), StandardCharsets.US_ASCII))) {
         return (T) pemParser.readObject();
      } catch (IOException e) {
         throw new ProxyException("Parse pem failed: " + pemFile, e);
      }
   }
   ...
   public static byte[] toPem(Object object) {
      ...
      try (PEMWriter writer = new PEMWriter(new OutputStreamWriter(outputStream))) {
         ...
         throw new ProxyException("Transform to pem failed: " + object, e);
      }
   }
   ...
   public static PrivateKeyInfo readPrivateKeyFromFile(String pemFile) {
      ...
      throw new ProxyException("Not a valid private key: " + pemFile);
   }
   ...
   /**
    * Used as a helper for creating server certificates which do not depend on openssl being installed. It does
    * overwrite existing certificates if they are present.
    *
    * @param caCertFile
    * @param caPrivateFile
    * @param keySize
